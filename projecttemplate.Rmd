...
geom_vline(aes(xintercept = CreditScore_mu), linetype = "dashed", color = 'darkred', size = 0.4)
a
#
...
# generating a mulitvariate plot of 'DebtToIncomeRatio' and 'CurrentDelinquencies' 
# using the 'IncomeRange_ordered' as a differentaitor with color
...
sp <- qplot(CurrentDelinquencies,
            ...
            data = multi_data_samp_f,
            color = factor(cut(CreditScoreMean,4)),
            alpha = I(0.2)) +
    xlim(NA, 40)
    # ylim(450,800)
sp
...
```{r,multivariate_Plot_k,fig.width=9,fig.height=20}
...
stat_smooth(method='lm',se=FALSE) +
    labs(x = 'Credit Score Mean (rating)', y = 'Current Delinquencies (counts)') +
...
sp <- qplot(CurrentDelinquencies,
            ...
            alpha = I(0.2)) +
    guides(color=guide_legend(override.aes = list(alpha = 1))) +
    labs(x = 'Current Delinquencies (counts)', y = 'Income Range ($)', color = 'Mean Credit Score') +
...
sp <- qplot(CurrentDelinquencies,
            ...
            alpha = I(0.2)) +
    guides(color=guide_legend(override.aes = list(alpha = 1))) +
    labs(x = 'Current Delinquencies (counts)', y = 'Income Category (categorical)', color = 'Mean Credit Score') +
